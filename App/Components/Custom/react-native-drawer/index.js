...
import {PanResponder, Animated, View, StyleSheet, Dimensions, InteractionManage} from "react-native";
...
export default class Drawer extends Component {
    ...
    initialize = (props) => {
        ...
        if (typeof this.props.isScale != 'undefined')   {
    ...
    updatePosition = () => {
        ...
        if (typeof this.props.isScale != 'undefined')   {
            this.setState({scale: 1 - 0.2 * ratio});
        }
    ...
    renderMain() {
        return (
            <Animated.View
                ...
                key="main"
                ref={c => this.main = c}
                style={[this.stylesheet.main, {backgroundColor: '#ccc', transform:[ {scale: this.state.scale}], height: this.getHeight(), width: this.getMainWidth()}]}
            >
                {this.props.children}
                <View
